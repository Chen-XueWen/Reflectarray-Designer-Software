...
%     csvwrite('ElementNum.csv',ElementNum)
    ...
    for j = 1:1:NumElement1axis
        for i = 1:1:NumElement1axis
            if DistributionData(j,i) > max(NormalizedPhase)
               Dimension = spline(NormalizedPhase,Dim,max(NormalizedPhase));
    ...
    Left = Coordinates(:,2) - Dimension/2;
    Top = Coordinates(:,1) + Dimension/2;
    Right = Coordinates(:,2) + Dimension/2;
    Bottom = Coordinates(:,1) - Dimension/2;
    Report = cat(2,Coordinates,Dimension);
    Report = cat(2,Report,Left);
    Report = cat(2,Report,Top);
    Report = cat(2,Report,Right);
    Report = cat(2,Report,Bottom);
    Script = fopen('ReflectarrayScript.scr','w');
    fprintf(Script,'RECTANGLE\n');
    
    for i = 1:1:TotalElement
        fprintf(Script,'%f,',Report(i,4));
        fprintf(Script,'%f\n',Report(i,5));
        fprintf(Script,'%f,',Report(i,6));
        fprintf(Script,'%f\n',Report(i,7));
        fprintf(Script,'\n');
    end
